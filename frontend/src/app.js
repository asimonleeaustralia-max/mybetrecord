...
const TOKEN_KEY = window.mbrStorage?.TOKEN_KEY || "mbr_token";
const storage = () => window.mbrStorage.safeStorage();
...
function confirmDialog(message) {
  ...
  $("#mbrModalMsg").textContent = message;
...
function promptDialog(message, defaultValue = "") {
  ...
  $("#mbrModalMsg").textContent = message;
  $("#mbrModalPrompt").hidden = false;
  const input = $("#mbrModalInput");
  ...
  return new Promise(resolve => {
    _modalResolve = (value) => resolve(value);
...
async function deleteBet(id) {
  if (!(await confirmDialog(t("bets.deleteConfirm")))) return;
...
async function revokeBetShare(id) {
  if (!(await confirmDialog(t("share.revokeConfirm")))) return;
  try {
...
async function renderForm(id) {
  ...
  ["odds", "odds_denominator", "personal_implied_odds", "model_implied_odds", "tipster_implied_odds"].forEach(n => {
...
function updateCalcBlock(form, kind, implied, oddsDec, bankroll, mult, currency, { kelly = true } = {}) {
  ...
  if (!kelly || !kellyWrap) return;
  const kellyResult = kellyFromImplied(oddsDec, implied, bankroll, mult);
  const showKelly = bankroll > 0 && oddsDec > 1;
  kellyWrap.hidden = !showKelly;
  if (showKelly && kellyVal) {
    kellyVal.textContent = kellyResult && kellyResult.fraction > 0
      ? t("form.kellySuggest", {
          stake: money(kellyResult.stake),
          currency,
          pct: (kellyResult.fraction * 100).toFixed(1),
        })
      : t("form.kellyNoEdge");
  }
}
...
function updateModellingCalcs(form = $("#betForm")) {
  ...
  const tipster = parseFloat(form.elements.namedItem("tipster_implied_odds")?.value);
  ...
  updateCalcBlock(form, "tipster", tipster, oddsDec, bankroll, mult, currency, { kelly: false });
}
...
const NUM_FIELDS = ["odds", "odds_denominator", "stake", "place_fraction", "cash_out_amount",
  "model_implied_odds", "personal_implied_odds", "tipster_implied_odds", "closing_odds", "closing_odds_exchange", "exchange_commission_pct"];
...
function fillForm(form, b) {
  ...
  set("tipster_implied_odds", b.tipster_implied_odds);
...
function renderChartFallback(canvas, rows, title) {
  ...
  if (!fb) {
    ...
    fb.className = "chart-fallback table-wrap";
    wrap.appendChild(fb);
  }
  fb.innerHTML = `
    <p class="hint">${esc(title)}</p>
    <table class="ledger ledger--compact">
      <thead><tr><th>${esc(t("reports.date"))}</th><th class="r">${esc(t("reports.value"))}</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>`;
}
...
function clearChartFallback(canvas) {
  canvas.hidden = false;
  canvas.closest(".chart-wrap")?.querySelector(".chart-fallback")?.remove();
}
...
async function loadEquity() {
  ...
  if (!chartReady || typeof Chart === "undefined") {
    const rows = pts.slice(-12).map(p =>
      `<tr><td>${esc(i18n.formatLocaleDate(new Date(p.date), { day: "2-digit", month: "short", year: "numeric" }))}</td><td class="r num">${money(p.cumulative)}</td></tr>`
    ).join("") || `<tr><td colspan="2" class="empty">${esc(t("reports.noData"))}</td></tr>`;
    renderChartFallback(ctx, rows, t("reports.chartFallback"));
    return;
  }
...
async function loadMonthly() {
  ...
  if (!chartReady || typeof Chart === "undefined") {
    const rows = keys.map(k =>
      `<tr><td>${esc(k)}</td><td class="r num">${money(byMonth[k])}</td></tr>`
    ).join("") || `<tr><td colspan="2" class="empty">${esc(t("reports.noData"))}</td></tr>`;
    renderChartFallback(ctx, rows, t("reports.chartFallback"));
    return;
  }
...
function renderProPlan(body, plan) {
  ...
  if (cancelBtn) cancelBtn.addEventListener("click", async () => {
    if (!(await confirmDialog(t("plan.cancelConfirm")))) return;
...
async function loadKeys() {
  ...
  $$("[data-revoke]").forEach(b => b.addEventListener("click", async () => {
    if (!(await confirmDialog(t("settings.revokeConfirm")))) return;
...
async function createKey() {
  const name = await promptDialog(t("settings.keyPrompt"), "default");
  if (name == null) return;
...
async function adminRemoveAdmin(userId) {
  if (!(await confirmDialog(t("admin.revokeAdminConfirm")))) return;
  try {
...
async function start() {
  try {
    if (window.mbrStorage?.isEphemeralStorage?.()) {
      toast(t("storage.ephemeral"));
    }
    if (window.__localeFallback) {
      toast(t("i18n.fallbackNotice"));
    }
    const shareToken = getShareToken();
    if (shareToken) {
